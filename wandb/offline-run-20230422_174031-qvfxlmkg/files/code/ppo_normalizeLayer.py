...
if __name__ == "__main__":
    ...
    np.savetxt(f"./model/BK{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}.txt",
               np.array(info["FD"][0]).view(float))
    np.savetxt(f"./model/RK{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}.txt",
               np.array(info["FD"][1]).view(float))
    np.savetxt(f"./model/BR{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}.txt",
