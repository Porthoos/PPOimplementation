...
def evaluata_policy(env, agent, normalize, gamma_):
    n = 3
    steps = 0
    mean, std = normalize.get()
    reward = 0
    for _ in range(n):
        done = False
        gamma = 1
        ...
        while not done:
            steps += 1
            action, logprob, _, value = agent.get_action_and_value(torch.unsqueeze(torch.Tensor(s), 0))
            s_, r, terminated, truncated, _ = env.step(torch.Tensor(action))
            s_ = (s_ - mean) / np.sqrt(1e-8 + std)
            s = s_
            reward += r * gamma
            gamma *= gamma_
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(torch.unsqueeze(next_obs, 0))
            ...
            if global_step % args.eval_freq == 0:
                eval_return, eval_length = evaluata_policy(evaluate_env, agent, envs, args.gamma)
                writer.add_scalar("eval/episode_return", eval_return, global_step)
                writer.add_scalar("eval/episode_length", eval_length, global_step)
                print(f"global_step={global_step}, episodic_return={eval_return}")
